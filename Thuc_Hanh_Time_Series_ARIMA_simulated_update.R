...
adf.test(socathang_ts) # Cần kiểm định lại tính dừng bằng KPSS
# Vẽ biểu đồ ACF và PACF trên cùng một hàng

library(tseries)

# Kiểm định KPSS cho tính dừng quanh một mức (Level Stationary)
kpss.test(socathang_ts, null = "Level")

library(tseries)

pp.test(socathang_ts)


# Kiểm định KPSS cho tính dừng quanh một xu thế (Trend Stationary) 
# Phù hợp với chuỗi có thành phần 600 + 2.5*t như bạn mô tả
kpss.test(socathang_ts, null = "Trend")

library(forecast)

# Kiểm tra số bậc sai phân thường (d) cần thiết dựa trên kiểm định KPSS
ndiffs(socathang_ts, test = "kpss")

# Kiểm tra số bậc sai phân mùa (D) cần thiết
nsdiffs(socathang_ts)

# Cần vẽ ACF và PACF trên chuỗi đã sai phân
# Sai phân bậc 1: y_t' = y_t - y_{t-1}
socathang_ts_d1 <- diff(socathang_ts, differences = 1)
# Vẽ biểu đồ ACF và PACF khi chưa sai phân
par(mfrow=c(1,2))
...
par(mfrow=c(1,2))
# Xác định thủ công p, d, q bằng biểu đồ ACF và PACF
acf(socathang_ts_d1, main="Biểu đồ ACF") # --> q = 2
pacf(socathang_ts_d1, main="Biểu đồ PACF") # --> p = 0

# Chuyển lại đơn vị Lag x theo ngày
par(mfrow=c(1,2))
acf(as.numeric(socathang_ts_d1), main="ACF", lag.max = 30) # Lag mặc định là 30
pacf(as.numeric(socathang_ts_d1), main="PACF", lag.max = 30) # Lag mặc định là 30
...
# ARIMA(0,1,2)
model_arima012 <- arima(socathang_ts, order = c(0, 1, 2))
summary(model_arima012)
BIC(model_arima012)
checkresiduals(model_arima012) # Không bác bỏ giả thuyết nhiễu trắng

# ARIMA(1,1,2)
model_arima112 <- arima(socathang_ts, order = c(1, 1, 2))
summary(model_arima112)
BIC(model_arima112)
checkresiduals(model_arima112) # Không bác bỏ giả thuyết nhiễu trắng

# ARIMA(0,1,1)
model_arima011 <- arima(socathang_ts, order = c(0, 1, 1))
summary(model_arima011)
BIC(model_arima011)
checkresiduals(model_arima011) # Không bác bỏ giả thuyết nhiễu trắng

# ARIMA(1,1,1)
model_arima111 <- arima(socathang_ts, order = c(1, 1, 1))
summary(model_arima111)
BIC(model_arima111)
checkresiduals(model_arima111) # Không bác bỏ giả thuyết nhiễu trắng

# ARIMA(1,2,2)
model_arima122 <- arima(socathang_ts, order = c(1, 2, 2))
summary(model_arima122)
BIC(model_arima122)
checkresiduals(model_arima122) # Không bác bỏ giả thuyết nhiễu trắng

# ARIMA(2,2,2)
model_arima222 <- arima(socathang_ts, order = c(2, 2, 2))
summary(model_arima222)
BIC(model_arima222)
checkresiduals(model_arima222) # Không bác bỏ giả thuyết nhiễu trắng
...
# Khớp mô hình ARIMA(0,0,2)(0,0,1)[12]
# Tham số order = c(p, d, q) -> (0, 0, 2)
# Tham số seasonal = c(P, D, Q) -> (0, 0, 1) với chu kỳ 12 (tháng)
model_sarima <- Arima(socathang_ts, 
                     order = c(0, 1, 2), 
                     seasonal = list(order = c(0, 1, 1), period = 12),
                     include.constant = TRUE) # Thêm hằng số nếu chuỗi có trung bình khác 0

# 3. Xem tóm tắt kết quả mô hình
summary(model_sarima)

# 4. Kiểm tra phần dư (Residuals) để đánh giá độ tin cậy
checkresiduals(model_sarima)

# Tham số seasonal = c(P, D, Q) -> (2, 1, 1) với chu kỳ 12 (tháng)
model_sarima_211 <- Arima(socathang_ts, 
                      order = c(0, 1, 2), 
                      seasonal = list(order = c(2, 1, 1), period = 12),
                      include.constant = TRUE) # Thêm hằng số nếu chuỗi có trung bình khác 0

# 3. Xem tóm tắt kết quả mô hình
summary(model_sarima_211)

# 4. Kiểm tra phần dư (Residuals) để đánh giá độ tin cậy
checkresiduals(model_sarima_211)

# Tham số seasonal = c(P, D, Q) -> (0, 1, 1) với chu kỳ 12 (tháng)
model_sarima_011 <- Arima(socathang_ts, 
                          order = c(0, 1, 1), 
                          seasonal = list(order = c(0, 1, 1), period = 12),
                          include.constant = TRUE) # Thêm hằng số nếu chuỗi có trung bình khác 0

# 3. Xem tóm tắt kết quả mô hình
summary(model_sarima_011)

# 4. Kiểm tra phần dư (Residuals) để đánh giá độ tin cậy
checkresiduals(model_sarima_011)

# Tham số seasonal = c(P, D, Q) -> (0, 1, 1) với chu kỳ 12 (tháng)
model_sarima_011211 <- Arima(socathang_ts, 
                          order = c(0, 1, 1), 
                          seasonal = list(order = c(2, 1, 1), period = 12),
                          include.constant = TRUE) # Thêm hằng số nếu chuỗi có trung bình khác 0

# 3. Xem tóm tắt kết quả mô hình
summary(model_sarima_011211)

# 4. Kiểm tra phần dư (Residuals) để đánh giá độ tin cậy
checkresiduals(model_sarima_011211)

# Tham số seasonal = c(P, D, Q) -> (0, 1, 1) với chu kỳ 12 (tháng)
model_sarima_011111 <- Arima(socathang_ts, 
                             order = c(0, 1, 1), 
                             seasonal = list(order = c(1, 1, 1), period = 12),
                             include.constant = TRUE) # Thêm hằng số nếu chuỗi có trung bình khác 0

# 3. Xem tóm tắt kết quả mô hình
summary(model_sarima_011111)

# 4. Kiểm tra phần dư (Residuals) để đánh giá độ tin cậy
checkresiduals(model_sarima_011111)

# Tham số seasonal = c(P, D, Q) -> (1, 1, 1) với chu kỳ 12 (tháng)
model_sarima_111111 <- Arima(socathang_ts, 
                             order = c(1, 1, 1), 
                             seasonal = list(order = c(1, 1, 1), period = 12),
                             include.constant = TRUE) # Thêm hằng số nếu chuỗi có trung bình khác 0

# 3. Xem tóm tắt kết quả mô hình
summary(model_sarima_111111)

# 4. Kiểm tra phần dư (Residuals) để đánh giá độ tin cậy
checkresiduals(model_sarima_111111)
...
# Sử dụng mô hình để dự báo 24 tháng tới
forecast_nhapviem_sarima_011211 <- forecast(model_sarima_011211, h = 24)
# Vẽ biểu đồ dự báo 3 tháng tiếp theo
plot(forecast_nhapviem_sarima_011211, main = "Dự báo  số trường hợp nhập viện trong 24 tháng tiếp theo", 
     xlab = "Thời gian", ylab = "Tổng ca nhiễm")

# Vẽ biểu đồ với đường dự báo màu đỏ
plot(forecast_nhapviem_sarima_011211, 
     main = "Dự báo số trường hợp nhập viện trong 24 tháng tiếp theo", 
     xlab = "Thời gian", 
     ylab = "Tổng ca nhiễm",
     lwd = 2,            # Đường dữ liệu gốc dày hơn
...
fitted_values <- fitted(model_sarima_011211)
